...
export default function OrderItems({ data }) {
  ...
  const currentStatus = STATUS_CONFIG[data.status] || STATUS_CONFIG.default;

  async function handleAction() {
    if (currentStatus.nextAction === "completed") {
      return navigate(`/waiter/tables/${data.tableId}`);
    }
  ...
  // 2. Helper to render the footer content
  const renderFooter = () => {
    if (loading) {
      return (
        <div className="w-full py-3 flex justify-center bg-slate-900 rounded-xl">
          <span className="h-5 w-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
        </div>
      );
    }

    // If status is Processing (Kitchen)
    ...
    // If status is Pending (Your Request) or Completed
    if (currentStatus.label) {
      return (
        <div className={`w-full py-2.5 text-center rounded-xl text-sm font-bold flex items-center justify-center gap-2 ${currentStatus.color}`}>
          {data.status === 'pending' && <Clock size={16} />}
          {currentStatus.label}
        </div>
      );
    }
