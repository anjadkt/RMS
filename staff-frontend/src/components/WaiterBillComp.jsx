import { Printer, MapPin, Calendar, Hash, ChevronRight} from 'lucide-react';
import { useEffect, useState } from 'react';

export default function WaiterBillComp({ data }) {
  const [imgLoad,setImgLoad] = useState(false);
  const [print,setPrint] = useState(null);

  const printBill = () => {
    if (data.qrImage && !imgLoad) return;
    setPrint(data._id);
  }
  ...
  return (
    <div key={data.billId} className="flex flex-col gap-6 justify-center items-center m-4">
      ...
      <div id={print === data._id ? 'bill-print' : null} className="bg-white w-lg max-w-xs border border-slate-200 rounded-sm overflow-hidden flex flex-col relative">
        ...
        <div className="px-6 py-2 lg:px-10 lg:py-4">
          ...
          <div className="flex flex-col gap-1 mb-6 text-[11px] lg:text-sm text-slate-500 font-mono border-y border-dashed border-slate-200 py-4">
            ...
            <div className='flex justify-between items-center'>
              ...
              <div className={`px-2 py-0.5 rounded text-[9px] font-bold tracking-widest bg-amber-100 text-amber-700`}>
