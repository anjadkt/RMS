...
export default function AdminTables() {
  ...
  // --- DUMMY DATA ---
  const dummyStaff = [
    { id: 1, name: "Rahul Sharma", photo: "https://api.dicebear.com/7.x/avataaars/svg?seed=Rahul" },
    { id: 2, name: "Suresh Kumar", photo: "https://api.dicebear.com/7.x/avataaars/svg?seed=Suresh" },
    { id: 3, name: "Anjali Devi", photo: "https://api.dicebear.com/7.x/avataaars/svg?seed=Anjali" },
  ];

  const filteredStaff = waiters.filter(s =>
    s.name.toLowerCase().includes(waiterSearch.toLowerCase())
  );
  ...
  async function fetchTables() {
    try {
      ...
      const newNumb = lastTable ? Number(lastTable.tableNumber.split("-")[1]) + 1 : 1;
  ...
  const createTable = async () => {
    ...
    try {
      const { data } = await api.post('/table/admin', { 
        tableNumber: number,
  ...
  return (
    <>
      ...
      <div className="fixed top-26 left-2 flex flex-col items-start gap-2 bg-white z-40">
        <button className={`flex items-center cursor-pointer justify-center gap-3 w-full text-sm font-bold py-2 px-2 rounded-sm bg-black/80 text-white shadow-lg transition-all`}>
          Tables
        </button>
      </div>
        ...
        <div className="flex flex-wrap gap-10">
          <div className="max-w-3xs w-full bg-white border border-gray-100 rounded-xl p-3 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between min-h-full">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                <h1 className="text-xl font-black text-gray-900 tracking-tight">New Table</h1>
              </div>
            </div>
            ...
            <div className="space-y-3 mb-4">
              ...
              {/* Waiter Search or Selected View */}
                ...
                {!selectedWaiter ? (
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="w-full bg-gray-50 border outline-none border-gray-600 rounded-xl py-3 pl-9 pr-4 text-sm font-bold transition-all"
                    />
                    {showDropdown && waiterSearch && (
                      <div className="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-xl max-h-40 overflow-y-auto">
                        {filteredStaff.map((staff) => (
                          <div
                            ...
                            className="px-4 py-2 hover:bg-gray-50 cursor-pointer text-sm font-semibold text-gray-700 flex items-center gap-3"
                          >
                            <img src={staff.details.photo} className="w-6 h-6 rounded-full bg-gray-100 object-cover" alt="" />
                            {staff.name}
                          </div>
                        ))}
                      </div>
                  ...
                  /* SELECTED WAITER BADGE */
                  <div className="flex items-center justify-between w-full bg-black text-white p-2 rounded-xl border border-black shadow-lg">
                    <div className="flex items-center gap-3">
                      <img 
                        src={selectedWaiter.details.photo} 
                        className="w-8 h-8 rounded-lg object-cover bg-white/20" 
                        alt="" 
                      />
                      <span className="text-xs font-black uppercase tracking-tight">{selectedWaiter.name}</span>
                    </div>
                    <button 
